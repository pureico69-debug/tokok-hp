...
export default function KatalogProdukPage() {
  ...
  // State Modal Konfirmasi Terjual Cepat
  ...
  // Fungsi Konfirmasi Terjual Cepat & Otomatis Masuk Transactions
  const confirmMarkAsSold = async () => {
    if (!selectedProductForSold) return;

    // 1. Update status produk menjadi 'sold'
    const { error: productError } = await supabase
      .from("products")
      .update({ 
        status: "sold", 
        is_featured: false, // Otomatis dicabut dari landing page
        updated_at: new Date().toISOString() 
      })
      .eq("id", selectedProductForSold.id);

    if (productError) {
      toast.error("Gagal menandai unit terjual: " + productError.message);
      return;
    }

    // 2. Buat kode transaksi unik otomatis (misal: TRX-YYYYMMDD-XXXX)
    const randomCode = Math.floor(1000 + Math.random() * 9000);
    const todayStr = new Date().toISOString().slice(0, 10).replace(/-/g, "");
    const transactionCode = `TRX-${todayStr}-${randomCode}`;

    // 3. Masukkan harga kotor ke tabel transactions
      ...
      .insert([
        {
          ...
          type: "penjualan",
          description: `Penjualan ${selectedProductForSold.name} ${selectedProductForSold.storage || ""} (${selectedProductForSold.imei || "-"})`,
          customer_or_vendor: "Customer Langsung",
          amount: Number(selectedProductForSold.price), // Harga kotor omset
    ...
    if (txError) {
      toast.error("Unit terjual, tapi gagal mencatat ke keuangan: " + txError.message);
    } else {
      toast.success(`Unit ${selectedProductForSold.name} terjual & omset Rp ${Number(selectedProductForSold.price).toLocaleString("id-ID")} otomatis tercatat!`);
    }
  ...
  return (
    <div className="min-h-screen bg-neutral-50/50 flex text-neutral-900 selection:bg-amber-200 selection:text-amber-900">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-neutral-200/80">
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-neutral-900">
                Katalog & Stok iPhone (Founder Portal)
              </h1>
              <p className="text-xs text-neutral-500 mt-1 font-mono">
                Kelola inventaris, data IMEI, kelengkapan, foto unit, dan status penjualan real-time.
              </p>
            </div>
                  ...
                  <tbody className="divide-y divide-neutral-100 text-xs">
                    {filteredProducts.map((product) => (
                      <tr key={product.id} className="hover:bg-neutral-50/50 transition-colors">
                        ...
                        <td className="p-4 font-mono space-y-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            {product.status === "sold" ? (
                              ...
                              <span className="px-2 py-0.5 rounded bg-sky-100 text-sky-700 font-bold text-[10px]">
                                Tersedia
                              </span>
                            )}
                          </div>
                        ...
                        <td className="p-4 pr-6 text-right font-mono space-x-2">
                          {/* Tombol Shortcut Cepat Tandai Terjual */}
                          {product.status !== "sold" && (
                            <button
                              onClick={() => {
                              ...
                              className="p-2 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 cursor-pointer transition-all"
                            >
      ...
      {/* Modal Konfirmasi Terjual Cepat */}
      {soldModalOpen && selectedProductForSold && (
        <div className="fixed inset-0 z-50 bg-neutral-950/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-neutral-200 rounded-3xl p-6 max-w-sm w-full space-y-4 shadow-2xl font-mono text-xs">
            <div className="flex items-center gap-3 text-emerald-600">
              <CheckCircle2 className="w-6 h-6 shrink-0" />
              <h3 className="font-bold text-sm text-neutral-900">Konfirmasi Unit Terjual</h3>
            </div>
            <p className="text-neutral-600">
              Apakah unit <strong className="text-neutral-900">{selectedProductForSold.name}</strong> dengan IMEI <strong className="text-neutral-900">{selectedProductForSold.imei}</strong> sudah lunas dan terjual? Omset sebesar <strong className="text-neutral-900">Rp {Number(selectedProductForSold.price).toLocaleString("id-ID")}</strong> akan otomatis tercatat ke keuangan.
            </p>
            <div className="flex items-center justify-end gap-2 pt-2">
              <button
                onClick={() => setSoldModalOpen(false)}
                className="px-3.5 py-2 rounded-xl border border-neutral-200 text-neutral-600 hover:bg-neutral-100 cursor-pointer"
              >
                Batal
              </button>
              <button
                onClick={confirmMarkAsSold}
                className="px-4 py-2 rounded-xl bg-emerald-600 text-white font-bold hover:bg-emerald-500 cursor-pointer shadow-md"
              >
                Ya, Tandai Terjual
              </button>
            </div>
          ...
          <div className="bg-white border border-neutral-200 rounded-3xl p-6 sm:p-8 max-w-2xl w-full space-y-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-neutral-100">
              ...
              <button
                ...
                className="text-xs font-mono text-neutral-400 hover:text-neutral-700"
              >
                [ Tutup ]
              </button>
            </div>
